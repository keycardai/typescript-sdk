...
import type { PrivateKeyring, IdentifiableKey } from "../keyring.js";
...
export class FilePrivateKeyStorage implements PrivateKeyStorage {
  ...
  async storeKeyPair(
    ...
    await fs.writeFile(this.#metadataPath(keyId), JSON.stringify(metadata, null, 2), {
      encoding: "utf-8",
      mode: 0o644,
    });
  }
  ...
  async loadKeyPair(
    keyId: string,
  ): Promise<{ privateKeyPem: string; publicKeyJwk: JsonWebKey }> {
    ...
    const metadata = JSON.parse(metadataRaw) as { public_key_jwk: JsonWebKey };
  ...
  async listKeyIds(): Promise<string[]> {
    try {
      ...
      for (const file of files.filter((f) => f.endsWith(".json"))) {
        ...
        if (await this.exists(keyId)) keyIds.push(keyId);
      }
      return keyIds.sort();
    } catch {
      return [];
